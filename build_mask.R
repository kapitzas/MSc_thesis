...
maskrast <- raster(vals = values(template), extent(template), res = res(template), crs = "+proj=longlat +ellps=GRS80 +no_defs")
...
cell_size <- raster::area(maskrast, na.rm=TRUE, weights=FALSE)
